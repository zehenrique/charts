...
kube_version=$(curl -fksS https://kubernetes.default.svc/version?timeout=32s)
echo "Found Kubernetes version info: $kube_version"
kube_version=$(echo "${kube_version}" | jq -r .gitVersion | cut -d- -f1)
echo "Extracted gitVersion: $kube_version"
